...
change if the stoxumd config and/or server are compromised.
...
*   sign tokens authorizing a stoxumd server to run as the validator identified
    by this public key.
...
The current token needs to be present in the stoxumd config file.
...
Sample output:
```
  Validator keys stored in /home/ubuntu/.stoxum/validator-keys.json
```
...
```
  Update stoxumd.cfg file with these values:
...
For a new validator, add the [validator_token] value to the stoxumd config file.
...
value. After the config is updated, restart stoxumd.
...
```
  ...
  Update stoxumd.cfg file with these values and restart stoxumd:
...
restart stoxumd. Rename the old key file and generate new [validator keys](#validator-keys) and
